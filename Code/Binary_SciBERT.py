...
pretrained_model = AutoModelForMaskedLM.from_pretrained("allenai/scibert_scivocab_uncased")
...
def experiment(seed):
	...
	df = pd.read_csv(r'Preprocessed_Final_OSTI.csv').dropna() #Change name here
	...
	#LOAD
	model = SciBERTClass("allenai/scibert_scivocab_uncased", LABELS)	
	...
	# Defining the training function on the 80% of the dataset for tuning the scibert model
	def train(epoch):
		tr_loss = 0
		n_correct = 0
		nb_tr_steps = 0
	...
	with open("SciBERT.txt", "a") as file:
		file.write("\n\SciBERT Result:\n\n")
