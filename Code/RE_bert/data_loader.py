...
class SemEvalProcessor(object):
    ...
    def _read_csv(cls, input_file):
        """Reads a CSV file with column headings."""
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = line['Text']  # Replace 'text_column_name' with the actual column name for text
            label = self.relation_labels.index(line['Relationship'])  # Replace 'label_column_name' with the actual column name for labels
            if i % 1000 == 0:
                logger.info(line)
