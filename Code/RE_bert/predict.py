...
def convert_input_file_to_tensor_dataset(
    ...
    df = pd.read_csv(pred_config.input_file)  # Read CSV file
...
def predict(pred_config):
    ...
    for batch in tqdm(data_loader, desc="Predicting"):
        ...
        with torch.no_grad():
            ...
            preds.extend(np.argmax(probabilities, axis=1))  # Store predictions
            probs.extend(probabilities)  # Store probabilities

    label_lst = get_label(args)
    column_headings = ['relation', 'entity', 'probability']

    with open(pred_config.output_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        ...
        for pred, prob in zip(preds, probs):
            writer.writerow([pred, label_lst[pred], max(prob)])  # Write the prediction, label, and max probability
