...
pretrained_model = AutoModelForMaskedLM.from_pretrained("markusbayer/CySecBERT")
config = BertConfig.from_pretrained("markusbayer/CySecBERT", output_hidden_states=True)
...
def experiment(seed):
	...
	encoder.fit(df['Domain'])
	df['enc_Domain'] = encoder.transform(df['Domain'])

	df = df[['Domain', 'preprocess_description', 'enc_Domain']]
	LABELS
	...
	class Triage(Dataset):
		...
		def __getitem__(self, index):
			sentence = str(self.data.preprocess_description[index])
	...
	class cySecBERTClass(torch.nn.Module):
	...
	#LOAD
	model = cySecBERTClass("markusbayer/CySecBERT", LABELS)	
	...
	# Defining the training function on the 80% of the dataset for tuning the cysecbert model
	def train(epoch):
		tr_loss = 0
		n_correct = 0
		nb_tr_steps = 0
	...
	def check_accuracy(loader, model):
		...
		with torch.no_grad():
			for i, data in enumerate(loader, 0):
				...
				y_scores.extend(F.softmax(scores, dim=1)[:, 1].cpu().numpy())
	...
	with open("SciSecBERT.txt", "a") as file:
		file.write("\n\SciSecBERT Result:\n\n")
		file.write(f"\n\nSeed Value: {experiment_seed}")
		file.write("\n\nClassification Report:\n\n")
		file.write(classification_report(y, y_pred))
